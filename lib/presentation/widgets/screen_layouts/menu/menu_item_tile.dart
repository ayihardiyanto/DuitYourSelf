...
class _MenuItemTileState extends State<MenuItemTile> {
  ...
  void initState() {
    super.initState();
    _animation =
        Tween<double>(begin: 220, end: 60).animate(widget.animationController);
    _sizedBoxAnimation =
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: <Widget>[
            Icon(
              widget.icon,
              color: widget.isSelected ? Purple.barneyPurple : Grey.brownGrey,
              size: 20,
            ),
            SizedBox(
              width: _sizedBoxAnimation.value,
            ),
            if (_animation.value >= 190)
              Text(
                widget.title,
              )
            else
